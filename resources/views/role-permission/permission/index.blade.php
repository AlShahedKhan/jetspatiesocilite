                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach ($permissions as $permission)
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                                    <td class="py-4 px-6 text-sm font-medium text-gray-900 dark:text-gray-200">{{ $permission->id }}</td>
                                    <td class="py-4 px-6 text-sm text-gray-700 dark:text-gray-300">{{ $permission->name }}</td>
                                    <td class="py-4 px-6 text-sm">
                                        <x-nav-link href="{{ route('permissions.edit', $permission->id) }}"
                                            class="inline-flex items-center px-3 py-1 bg-yellow-500 text-white text-xs font-medium rounded-md hover:bg-yellow-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition"
                                        >
                                            {{ __('Edit') }}
                                        </x-nav-link>
                                        <x-nav-link href="{{ route('permissions.destroy', $permission->id) }}"
                                            class="inline-flex items-center px-3 py-1 ml-2 bg-red-600 text-white text-xs font-medium rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition"
                                        >
                                            {{ __('Delete') }}
                                        </x-nav-link>
                                    </td>
